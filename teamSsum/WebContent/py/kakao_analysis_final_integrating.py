...
f=open("C:\\Users\\pc-06\\Desktop\\WebServer\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp3\\wtpwebapps\\teamSsum\\FileSavedFolder\\KakaoTalk.txt",'r',encoding="utf-8")
...
for i in range(len(my_talk)):
    for j in range(len(All_time_list)):
        my_talk[i]=my_talk[i].replace(my_name,"").replace(All_time_list[j],"").replace('\n',"")
